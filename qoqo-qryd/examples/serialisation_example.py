...
pauli_product_measurement = PauliZProduct(
    constant_circuit=init_circuit,
    circuits=[z_circuit, x_circuit],
    input=measurement_input,
)
register_measurement = ClassicalRegister(
    constant_circuit=init_circuit, circuits=[z_circuit, x_circuit]
)

# Serialisation of Program returning register measurements with postprocessing
program = QuantumProgram(
    measurement=register_measurement, input_parameter_names=["angle"]
)
...
program = QuantumProgram(
    measurement=pauli_product_measurement, input_parameter_names=["angle"]
)
program_json = program.to_json()
# Trick to make the json easier to read
json_dict = json.loads(program_json)
...
# writing json to file
with open(
    tmp / "serialised_quantum_program_measuring_expectation_values.json", "w"
) as f:
    f.write(json_pretty_string)
